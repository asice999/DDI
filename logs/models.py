...
class OperationLog(models.Model):
    """操作日志 - 记录用户在系统中的所有关键操作（增删改查/导入导出/登录退出）"""
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True,
                             related_name='operation_logs', verbose_name='操作用户')  # 用户删除后保留日志
    ...
    old_value = models.TextField('变更前内容', blank=True)  # 用于审计对比
    new_value = models.TextField('变更后内容', blank=True)  # 用于审计对比
